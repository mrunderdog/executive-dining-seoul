...
# Merchant spellings in the official expense files are not always stable. These are
# only normalization rules for records that have been checked as the same outlet.
ENTITY_ALIASES = {
    "일산하인선생": "하인선생 일산점",
    "하인선생": "하인선생 일산점",
    "우설화": "우설화 일산점",
...
# Business information is deliberately separated from public-expense evidence. These
# overrides are used only when a matching current/local business source was checked.
ENTITY_OVERRIDES = {
    "고봉산한우터": {
        "address": "경기도 고양시 일산동구 성석로 17",
        "phone": "031-975-9234",
        "category": "고기·구이 · 한우",
        "url": "https://place.udanax.org/p/12167/%EA%B3%A0%EB%B4%89%EC%82%B0%ED%95%9C%EC%9A%B0%ED%84%B0",
    },
    "원주추옥": {
        "address": "경기도 고양시 덕양구 원당로33번길 58",
        "phone": "031-968-7888",
        "category": "국물·탕 · 추어탕",
        "url": "https://www.diningcode.com/profile.php?rid=TREuf7DSQEq9",
    },
    "맛있는정원": {
        "address": "경기도 고양시 덕양구 성사동 338-20",
        "phone": "031-967-9888",
        "category": "고기·구이",
        "url": "",
    },
    "사계절어촌": {
        "address": "경기도 고양시 일산서구 주엽로 80",
        "phone": "031-929-6765",
        "category": "회·해산물",
        "url": "",
    },
    "화정가든": {
        "address": "경기도 고양시 덕양구 행주로15번길 49-12",
        "phone": "031-972-7705",
        "category": "한식 · 보리굴비/홍어",
        "url": "https://www.hjgarden.net/main/index.html",
    },
    "일산긴자": {
        "address": "경기도 고양시 일산동구 무궁화로 359",
        "phone": "",
        "category": "일식",
        "url": "",
    },
    "일산들밥": {
        "address": "경기도 고양시 일산서구 중앙로 1581",
        "phone": "031-922-6862",
        "category": "한식 · 한정식",
        "url": "",
    },
    "엄선 엄마의 생선찜 구이": {
        "address": "경기도 고양시 일산서구 대산로212번길 7-6",
        "phone": "",
        "category": "해물·생선",
        "url": "https://bizno.net/article/3631602079",
    },
    "오대돌솥추어탕": {
        "address": "경기도 고양시 일산서구 대화로 150-3",
        "phone": "031-911-2200",
        "category": "국물·탕 · 추어탕",
        "url": "https://www.diningcode.com/profile.php?rid=06lUMa5DF9nO",
    },
    "고양회관": {
        "address": "경기도 고양시 덕양구 고양시청로 16-5",
        "phone": "",
        "category": "한식",
        "url": "https://opengo.kr/5601fb7e0e887edf2cf45ac3",
    },
    "남궁": {
        "address": "경기도 고양시 일산서구 대화동 2101",
        "phone": "031-911-3773",
        "category": "중식",
        "url": "",
    },
    "하인선생 일산점": {
        "address": "경기도 고양시 일산동구 식사동 833-14",
        "phone": "031-968-9819",
        "category": "중식",
        "url": "",
    },
    "우설화 일산점": {
        "address": "경기도 고양시 일산동구 식사동 833-14",
        "phone": "031-968-9401",
        "category": "고기·구이 · 한우/갈비",
        "url": "",
    },
    "보양삼계탕": {
        "address": "경기도 고양시 일산서구 주화로 180",
        "phone": "031-912-9911",
        "category": "국물·탕 · 삼계탕",
        "url": "",
    },
    "교촌치킨(강선점)": {
        "address": "경기도 고양시 일산동구 정발산동 687-5",
        "phone": "031-911-9980",
        "category": "치킨",
        "url": "",
    },
    "송포털레기 추어탕": {
        "address": "경기도 고양시 일산서구 강성로214번길 8-22",
        "phone": "031-916-8252",
        "category": "국물·탕 · 추어탕/매운탕",
        "url": "https://www.diningcode.com/profile.php?rid=gwD2hIJr83yt",
    },
    "진국한식부페": {
        "address": "경기도 고양시 덕양구 고양시청로 5",
        "phone": "",
        "category": "한식 · 한식뷔페",
        "url": "https://www.diningcode.com/profile.php?rid=8RrQuUfVlLi6",
    },
}
...
def _destination_score(visits: int, roles: int, months: int, evening_ratio: float, outside_ratio: float) -> float:
    ...
    exec_norm = visit_norm
    role_norm = min(roles / 4, 1.0)
    month_norm = min(months / 5, 1.0)
    return round(100 * (0.25*distance + 0.25*visit_norm + 0.20*exec_norm + 0.15*role_norm + 0.10*month_norm + 0.05*evening_ratio), 1)
...
def _build_source_records(source: str, spec: dict) -> list[dict]:
    ...
    # Aggregate spelling variants before applying the publication gate.
    ...
    for rank, c in enumerate(selected, start=1):
        ...
        if has_destination: why += f", 고양시 밖 주소 사용 비중 {round(outside_ratio*100)}%"
        why += "이 확인됩니다."
...
def merge_published_sources(payload: dict) -> dict:
    base = list(payload.get("records", [])); seen = {(str(r.get("name","")).strip(),str(r.get("origin","")).strip()) for r in base}; added=[]
    for source,spec in SOURCE_SPECS.items():
        for r in _build_source_records(source,spec):
            k=(r["name"],r["origin"])
            if k in seen: continue
            seen.add(k); added.append(r)
    records=base+added; payload=dict(payload); payload["records"]=records
    ...
    meta=dict(payload.get("meta") or {}); meta["published_supplements"]={source:sum(1 for r in added if r.get("published_source")==source) for source in SOURCE_SPECS}; meta["scope"]="서울·경기·인천 수도권 기초의회 (지원 source부터 단계적 자동 게시)"; meta["source_scope"]="서울 자치구의회 seed + 검증·정규화된 수도권 기초의회 업무추진비"
    if added:
