...
EXCLUDE=("경조","축의","조의","화환","꽃","주유","주차","택시","교통","온라인","쿠팡","문구","물품","기념품","사무용","상품권","커피","카페","다과","간식")
...
def meal(r):
    ...
    return bool(m) and not any(x in c for x in EXCLUDE) and (any(x in p for x in MEAL_WORDS) or any(x in t(r.get("role")) for x in SENIOR))
...
def main():
    ...
    for (name,address),items in groups.items():
        ...
        rc=Counter(t(r.get("role")) or t(r.get("department")) or "직위 미상" for r in items);ic=Counter(t(r.get("institution")) for r in items if t(r.get("institution")))
